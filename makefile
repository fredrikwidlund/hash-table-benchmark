VARIANTS_STR	= standard_str dynamic_str densehash_str ulib_str tommyds_str
VARIANTS_INT	= standard_int dynamic_int densehash_int ulib_int tommyds_int
...
SUPPORT_SRCS	= support/tommyds/tommylist.c support/tommyds/tommyhashdyn.c support/ulib/hash_func.c
...
$(VARIANTS_STR): template_str.cc
	$(CXX) $(CXXFLAGS) $(SUPPORT_SRCS) -o $@ template_str.cc $(LDLIBS) -D$$(echo $@ | tr '[:lower:]' '[:upper:]')

$(VARIANTS_INT): template_int.cc
	$(CXX) $(CXXFLAGS) $(SUPPORT_SRCS) -o $@ template_int.cc $(LDLIBS) -D$$(echo $@ | tr '[:lower:]' '[:upper:]')
