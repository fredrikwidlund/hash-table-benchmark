...
data.densehash <- read.csv("densehash_str.dat", head=TRUE, sep=",")
data.dynamic <- read.csv("dynamic_str.dat", head=TRUE, sep=",")
data.standard <- read.csv("standard_str.dat", head=TRUE, sep=",")
data.tommyds <- read.csv("tommyds_int.dat", head=TRUE, sep=",")
data.ulib <- read.csv("ulib_str.dat", head=TRUE, sep=",")

graph <- ggplot(legend = TRUE) + 
  ggtitle('String hash table benchmark') +
...
ggsave(graph, file = "hash-table-str-benchmark.pdf", width = 10, height = 5)
